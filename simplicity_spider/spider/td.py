...
class Td(Base_spider):
    def __init__(self, is_proxies=None, hw_db=None, kafka_pro=None):
        super(Td, self).__init__(is_proxies=is_proxies)
        self.headers = {
    ...
    def video_del(self, the_xpath, tree, entity_url):
        ...
        for v in tree.xpath(the_xpath):
            if re.search(r"(?<=youtube-nocookie.com/embed/).*?(?=\?wmode)", v):
                video_li.append(
                    "https://www.youtube.com/watch?v=" + re.search(r"(?<=youtube-nocookie.com/embed/).*?(?=\?wmode)",
                                                                   v).group())
        return video_li
    ...
    def entity_spider(self, entity_url):
        try:
            ...
            title_xpath = "//h1[contains(@class,'dcr-')]"
            ...
            pubtime_css = "//summary[@class='dcr-1ybxn6r']/span[@class='dcr-u0h1qy']"
            ...
            img_css = "//article//figure/div/picture/img"
            video_xpath = "//div[@id='maincontent']/div/figure//iframe/@src"
...
if __name__ == '__main__':
    ...
    hw_obs = HwObs()
    mp = Td(is_proxies=proxies, hw_db=hw_obs, kafka_pro=producer)
    mp.id_split()
